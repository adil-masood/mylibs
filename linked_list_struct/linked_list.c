...
void list_insert(point *ptr,student *self,int pos){ //pos = -1 to place at the end
    if(pos>-1){
        ...
        data->name = (char *)malloc(strlen(self->name));
        data->section = (char *)malloc(strlen(self->section));
        data->class = (char *)malloc(strlen(self->class));
...
void list_pop(point *ptr){
    if(ptr->head != NULL){
        if(ptr->head->link == NULL){
            free(ptr->head->data->name);
            free(ptr->head->data->section);
            free(ptr->head->data->class);
            free(ptr->head->data);
            free(ptr->head);
            ptr->head=NULL;
        }
            ...
            free(ptr->current->link->data->name);
            free(ptr->current->link->data->section);
            free(ptr->current->link->data->class);
            free(ptr->current->link->data);
    ...
    if(pos>-1){
        if(ptr->head != NULL){
            if(pos == 0){
                ...
                free(temp->data->name);
                free(temp->data->section);
                free(temp->data->class);
                free(temp->data);
                free(temp);
            }
            else{
